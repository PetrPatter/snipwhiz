...
- Produces:
  ...
  - `CapturePipeline(CaptureStore store)` with `Complete(FrozenDesktop, PixelRect region, string sourceApp, string sourceTitle) -> CaptureOutcome`
...
public sealed class CapturePipeline(CaptureStore store)
{
